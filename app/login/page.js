...
function LoginForm() {
  ...
  const afterLogin = safeAfterLoginPath(searchParams.get('callbackUrl'))
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
        ...
        <form onSubmit={handleCredentials} className="space-y-4 mb-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 bg-black text-white rounded-xl font-medium text-sm hover:bg-gray-800 transition disabled:opacity-60"
          >
        ...
        <button
          onClick={() => signIn('google', { callbackUrl: afterLogin })}
          className="w-full flex items-center justify-center gap-3 py-2.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
        >
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
      <Suspense
        fallback={
          <div className="w-full max-w-md h-64 flex items-center justify-center text-sm text-gray-500">Loading…</div>
        }
      >
        <LoginForm />
      </Suspense>
